 ...
 * and click are already spoken for by the act of choosing someone — and out of
 * `RespondentsList`, where hovering a name highlights that member's
 * availability in the grid a card would then cover.
 */
import { mapGetters } from "vuex"
import UserAvatarContent from "@/components/UserAvatarContent.vue"
import { formatPhone, isTouchEnabled, userFromDisplayName } from "@/utils"
import { personDetail } from "@/utils/directory"
...
export default {
  ...
  computed: {
    ...
    detail() {
      const record =
        this.person ?? (this.userId ? this.personById(this.userId) : null)
      ...
      const seed = this.userId
        ? { ...base, _id: base?._id ?? this.userId }
        : base
  ...
  methods: {
     ...
     */
    prefetch() {
      if (this.person || !this.userId) return
      this.$store.dispatch("ensurePerson", this.userId)
    },
  },
}
</script>
