 ...
 *
 * Extracted verbatim from ScheduleOverlap.vue as a Vue 2 mixin (TODO A5). Mixin
 * methods run against the same component instance, so every `this.*` reference
 * (data, computed, other methods, DRAG_TYPES, etc.) resolves exactly as before
 * — this is a behavior-preserving move, not a rewrite. The component keeps all
 * the state these methods read/write.
 */
export default {
  methods: {
    normalizeXY(e) {
